...
$tg_chatid = '-4251179444'; // ⚠️ для групи часто треба -100...
...
if (!empty($phone)) {
  ...
  if (function_exists('curl_init')) {
    ...
    if ($res !== false && $code === 200) {
      ...
      $tg_error = "HTTP {$code}. cURL: {$err}. Response: {$res}";
    }
  } else {
    $tg_error = "На хостингу нема cURL (curl_init недоступний).";
  }

  // Лог у файл (щоб не гадати)
  $logLine = "[".$date."] sent=" . ($tg_sent ? "1" : "0") . " chat_id={$tg_chatid} phone={$phone} error={$tg_error}\n";
  @file_put_contents(__DIR__ . '/tg_log.txt', $logLine, FILE_APPEND);
} else {
  $tg_error = "Телефон порожній (phone не прийшов).";
}
?>
<!DOCTYPE html>
<html lang="uk">
...
.note{font-size:12px;color:#666;margin-top:10px;word-break:break-word}
</style>
...
<div class="box">
  ...
  <?php if (!$tg_sent): ?>
    <div class="note"><b>Telegram не відправив:</b> <?= htmlspecialchars($tg_error) ?><br>
    <b>Лог:</b> raundap_files/tg_log.txt</div>
  <?php endif; ?>
</div>
